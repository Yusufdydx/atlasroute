...
class FindClinicsTool implements IMcpTool {
  registerTool(server: McpServer, _req: Request) {
    server.registerTool(
      "FindClinics",
      {
        description:
          "Finds medical clinics near a location. " +
        ...
        inputSchema: {
          location: z.string().describe("The location to search near (address, city, or lat,lon)"),
      ...
      async ({ location, radius, limit }) => {
        try {
          ...
          const facilities = await OverpassUtilities.findNearbyFacilities(
            geoResult.latitude, geoResult.longitude, searchRadius, ["clinic"]
          );
...
export const FindClinicsToolInstance = new FindClinicsTool();
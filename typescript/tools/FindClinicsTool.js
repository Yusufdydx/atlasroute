    ...
    registerTool(server, _req) {
        server.registerTool("FindClinics", {
            ...
            try {
                ...
                const facilities = yield overpass_1.OverpassUtilities.findNearbyFacilities(geoResult.latitude, geoResult.longitude, searchRadius, ["clinic"]);
                const results = facilities.slice(0, maxResults).map((f) => ({
                    name: f.name, address: f.address, phone: f.phone, openingHours: f.openingHours, amenities: f.amenities, latitude: f.latitude, longitude: f.longitude
                }));
                return mcp_utilities_1.McpUtilities.createTextResponse(JSON.stringify({
                    ...
                    results: results, searchRadius: searchRadius, totalFound: facilities.length, facilityType: "clinic",
...
exports.FindClinicsToolInstance = new FindClinicsTool();

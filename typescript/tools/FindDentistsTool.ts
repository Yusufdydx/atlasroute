...
class FindDentistsTool implements IMcpTool {
  registerTool(server: McpServer, _req: Request) {
    server.registerTool(
      "FindDentists",
      {
        description:
          "Finds dental clinics near a location. " +
      ...
      async ({ location, radius, limit }) => {
        try {
          ...
          const facilities = await OverpassUtilities.findNearbyFacilities(
            geoResult.latitude, geoResult.longitude, searchRadius, ["dentist"]
          );
...
export const FindDentistsToolInstance = new FindDentistsTool();